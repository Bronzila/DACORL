...
do
    ...
    if [ -z "$TEACHER_DIR" ]; then
        ...
        TEACHER_ARG="--teacher_dir $TEACHER_DIR/SGD/$teacher/$function/"
    fi
    # TITLE="Comparing Teacher and Agent Trajectories on $function"
    TITLE=""

    python -W ignore plotting.py --data_dir $RESULTS_DIR/SGD/$teacher/ $TEACHER_ARG --comparison --teacher --teacher_label "$LABEL" --agent_labels "TD3+BC" --title "$TITLE" --heterogeneous --metric "valid_acc"
