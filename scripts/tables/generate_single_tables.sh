...
then
    ...
    python generate_latex_tables.py --path data_single_64/ToySGD/ --lowest --mean --auc
elif [ 2 -eq $SLURM_ARRAY_TASK_ID  ]
then
    echo "Single Teacher Agent Table";
    python generate_latex_tables.py --path data_single_64/ToySGD/ --lowest --mean --auc --results --multi_seed --num_runs 1000
fi