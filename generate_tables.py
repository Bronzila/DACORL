...
if __name__ == "__main__":
    ...
    if base_path.name == "ToySGD":
        ...
        raise NotImplementedError(
            f"Currently the benchmark {base_path.name} is not implemented."
        )
    ...
    for function in args.functions:
        for agent_id in args.ids:
            header = [" "]
            header.extend(
                [teacher_mapping[teacher] for teacher in args.teacher]
            )
            rows_mean = [header]
            rows_iqm = [header]
            rows_auc = [header]
            ...
            for i, agent in enumerate(args.agents):
                agent_str = (
                    f"\\acrshort{{{agent}}}" if agent != "teacher" else agent
                )
                row_mean = [agent_str]
                row_iqm = [agent_str]
                row_auc = [agent_str]
                ...
                for j, teacher in enumerate(args.teacher):
                    ...
                    if args.mean:
                        mean_str = (
                            f"{format_number(mean)} {pm} {format_number(std)}"
                        )
            ...
            if args.mean:
                # Regular mean
                table_result_path = generate_file_path(
                    base_path, "mean", function, agent_id, args.format
                )
                ...
                # IQM
                table_result_path = generate_file_path(
                    base_path, "iqm", function, agent_id, args.format
                )
            ...
            if args.lowest:
                table_result_path = generate_file_path(
                    base_path, "lowest", function, agent_id, args.format
                )
                table_content = generate_table(
                    rows_lowest, args.format, lowest_min
                )
                with table_result_path.open("w") as f:
                    f.write(table_content)
            if args.auc:
                table_result_path = generate_file_path(
                    base_path, "auc", function, agent_id, args.format
                )
