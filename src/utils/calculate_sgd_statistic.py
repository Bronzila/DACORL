...
def calculate_single_seed_statistics(calc_mean=True, calc_lowest=True, n_lowest=1, path=None,
                                    results=True, verbose=False, interpolation=False, calc_auc=True, metric="test_acc"):
    paths = []
    ...
    return max_mean, max_std, lowest_vals_of_max_mean, max_iqm, max_iqm_std, max_path, max_auc, max_auc_std

def calculate_multi_seed_statistics(calc_mean=True, calc_lowest=True, n_lowest=1, path=None,
                                    results=True, verbose=False, num_runs=100, interpolation=False, calc_auc=True, metric="test_acc"):
    seed_dirs = set()
    filename = "eval_data_interpolation.csv" if interpolation else "eval_data.csv"
    if results:
    ...
    for seed_dir in seed_dirs:
        max_mean, max_std, _, _, _, max_path, _, _ = calculate_single_seed_statistics(calc_mean, calc_lowest,
    ...
    if calc_lowest:
        lowest_vals = find_highest_values(combined_data, metric, n_lowest)[metric]
    if calc_auc:
...
def calculate_statistics(calc_mean=True, calc_lowest=True, n_lowest=1, path=None,
                         ...
                         interpolation=False, calc_auc=True, metric="test_acc"):
    if multi_seed:
