...
def compare_experiments(exp1_path, exp2_path):
    experiments_paths = [Path(exp1_path), Path(exp2_path)]
    tables_path = "tables"
    
    statistics_types = ['iqm']#, 'iqm', 'lowest']
    functions = ["Sphere"] #'Ackley', 'Rastrigin', 'Rosenbrock', 'Sphere']

    results = {stat_type: {func: {} for func in functions} for stat_type in statistics_types}
    
    for stat_type in statistics_types:
        for func in functions:
            file_name = f"{stat_type}_{func}_0.md"
            file_paths = [exp / "ToySGD"/ tables_path / file_name for exp in experiments_paths]
            ...
            if all(file_path.exists() for file_path in file_paths):
                ...
                common_agents = set(df1.iloc[:, 0]).intersection(set(df2.iloc[:, 0])) - {'teacher'}
                    ...
                    for agent in common_agents:
                        ...
                        for teacher in teachers_1[1:]:
                            ...
                            if std1 is not None and std2 is not None:
                                val_change = calculate_percentage_change(val1, val2)
                                std_change = calculate_percentage_change(std1, std2)
                                results[stat_type][func].setdefault(agent, {}).setdefault(teacher, []).append(val_change)
                                results[stat_type][func].setdefault('overall', {}).setdefault(teacher, []).append(val_change)
                            else:
                                val_change = calculate_percentage_change(val1, val2)
                                results[stat_type][func].setdefault(agent, {}).setdefault(teacher, []).append(val_change)
                                results[stat_type][func].setdefault('overall', {}).setdefault(teacher, []).append(val_change)
                else:
                    print(f"No common agents found for {stat_type.upper()} - {func} excluding 'teacher'\n")
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compare experiment results between two experiments.")
    parser.add_argument("exp1_path", type=str, help="Path to the first experiments folder.")
    parser.add_argument("exp2_path", type=str, help="Path to the second experiments folder.")
    args = parser.parse_args()
    
    compare_experiments(args.exp1_path, args.exp2_path)
